...
def not_edge_case(line):
    if "website" in line:
        print(line.replace(" ", ""), "|website=" in line.replace(" ", ""))
    if "|website=" in line.replace(" ", "") and not "*" in line and not "{{" in line and not "[" in line and not "<!--" in line and not "<ref>" in line and not "</ref>" in line and not "url=" in line and not "|" in line and "http" in line:
        return True
    else:
        return False
